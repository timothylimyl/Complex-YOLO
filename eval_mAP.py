...
if __name__ == "__main__":
    ...
    parser.add_argument("--metric", type=str, default="normal", help="type COCO for COCO metric of IoU")
    ...
    if opt.metric == 'COCO':# COCO style:
        
        precision_lst = []
        AP_lst        = []
        
        # Evaluate through different thresholds
        for iou_thres in np.arange(0.5,0.951,0.05):
            ...
            AP_lst.append(AP) #get each AP based on the IoU Thresholds
            
            
        #print(AP_lst) #Comment out, just checking expected output
        ...
        AP = [AP_1/len(AP_lst),AP_2/len(AP_lst),AP_3/len(AP_lst)]
        
        print("Average Precisions:")
        for c in range(0,3):
            print(f"+ Class '{c}' ({class_names[c]}) - AP: {AP[c]}")
                     
        print(f"mAP: {sum(AP)/len(AP)}") 
        print(f"COCO Metrics. mAP averaged over 10 IoU thresholds")
